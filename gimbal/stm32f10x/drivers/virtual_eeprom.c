...
//动态给定虚拟地址
uint16_t InitVirtAddVarTab(void)
{
	uint16_t i = 0;
	for(i=0;i<NumbOfVar;i++)
	{
		VirtAddVarTab[i] = VirtAddSTA+i;
	}
  ...
  switch (PageStatus0)
  {
    case ERASED:
      if (PageStatus1 == VALID_PAGE) /* Page0 erased, Page1 valid */
      {
        /* Erase Page0 */
				#if defined (STM32F4XX)
				FlashStatus = FLASH_EraseSector(GetSector(PAGE0_BASE_ADDRESS), VoltageRange_3);
				#else
    ...
    case RECEIVE_DATA:
      if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
      {
        ...
        /* Erase Page1 */
				#if defined (STM32F4XX)
				FlashStatus = FLASH_EraseSector(GetSector(PAGE1_BASE_ADDRESS), VoltageRange_3);
				#else
...
uint16_t EE_ReadVariable(uint16_t VirtAddress, uint16_t* Data)
{
  ...
  /* Get valid Page for write operation */
  ValidPage1 = EE_FindValidPage(WRITE_IN_VALID_PAGE);
	
  /* Check if there is no valid page */
  if (ValidPage == NO_VALID_PAGE && ValidPage1 == NO_VALID_PAGE)
  {
    return  NO_VALID_PAGE;
  }
...
static uint16_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data)
{
  ...
  /* Erase the old Page: Set old Page status to ERASED status */
	#if defined (STM32F4XX)
	FlashStatus = FLASH_EraseSector(GetSector(OldPageAddress), VoltageRange_3);
	#else
