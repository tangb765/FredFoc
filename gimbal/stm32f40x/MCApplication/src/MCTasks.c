...
struct rt_semaphore hMFTask1_sem;
struct rt_semaphore hMFTask2_sem;
static volatile uint16_t hMFTaskCounterM1_t = 0;
static volatile uint16_t hMFTaskCounterM2_t = 0;
...
{
	rt_memcpy((void *)&configs.oPIDPos,&PIPosParamsM1,sizeof(PIParams_t));
	rt_memcpy((void *)&configs.oPIDDPos,&PIDPosParamsM1,sizeof(PIDParams_t));
	rt_memcpy((void *)&configs.oPIDSpeed,&PISpeedParamsM1,sizeof(PIParams_t));
	rt_memcpy((void *)&configs.oPIDIq,&PIIqParamsM1,sizeof(PIParams_t));
	rt_memcpy((void *)&configs.oPIDId,&PIIdParamsM1,sizeof(PIParams_t));
}

void Init_PID(void)
{
	rt_memcpy((void *)&PIPosParamsM1,&configs.oPIDPos,sizeof(PIParams_t));
	rt_memcpy((void *)&PIDPosParamsM1,&configs.oPIDDPos,sizeof(PIDParams_t));
	rt_memcpy((void *)&PISpeedParamsM1,&configs.oPIDSpeed,sizeof(PIParams_t));
	rt_memcpy((void *)&PIIqParamsM1,&configs.oPIDIq,sizeof(PIParams_t));
	rt_memcpy((void *)&PIIdParamsM1,&configs.oPIDId,sizeof(PIParams_t));
}
...
void MCboot(CMCI oMCIList[NBR_OF_MOTORS],CMCT oMCTList[NBR_OF_MOTORS])
{
  ...
  HFI_FP_Init_t HFI_FP_InitStructureM2; /* only if M2 has HFI */
	...
	rt_sem_init(&hMFTask1_sem, "MFTask1", 0, 0);
	rt_sem_init(&hMFTask2_sem, "MFTask2", 0, 0);
...
#error "Invalid boot: configuration not valid or to be assigned"
  ...
  STM_Init(oSTM[M1]);
	...
	oPIDPos[M1] = (CPID_PI)PID_NewObject(&PIPosParamsM1,&PIDPosParamsM1);
	PID_ObjectInit(oPIDPos[M1]);
  ...
  MCTInitStruct.oPIDPos = oPIDPos[M1];
...
volatile uint8_t MotorMode = 251;
/**
...
void TSK_MediumFrequencyTaskM1(void)
{
...
#endif
    if(STM_NextState(oSTM[M1], IDLE) == TRUE)
    {
      FOC_Clear(M1);
  ...
  case START:
    {    
      ...
      STM_NextState(oSTM[M1], START_RUN); /* only for sensored*/  //DRF-UPD
#endif
...
#if ((defined ENCODER)||(defined VIEW_ENCODER_FEEDBACK)) /*  only for encoder*/
  case IDLE:
    if (EAC_GetRestartState(oEAC_M2))
    {
      EAC_SetRestartState(oEAC_M2,FALSE); /* Reset restart flag*/
      STM_NextState(oSTM[M2],IDLE_START);
    }
		else
		{
...
{ 
  ...
  SPD_CalcAngle(oSpeedSensor[M1],MC_NULL, EAC_GetAligned_num(oEAC_M1), bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/    
#endif
  ...
  else
  {
    ...
    SPD_CalcAngle(oSpeedSensor[M1],&STO_Inputs, 0, bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/
    ...
    if((hState == START) || (hState == START_RUN)) /*  only for sensor-less*/
    {
      ...
      SPD_CalcAngle(oVSS_M1,&hObsAngle, 0, bMotorNbr);
    }
#endif
...
#if ((defined ENCODER)||(defined VIEW_ENCODER_FEEDBACK))
    SPD_CalcAngle(oSpeedSensor[bMotorNbr],MC_NULL, EAC_GetAligned_num(oEAC_M1), bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/
#endif
		
#if ((defined HALL_SENSORS)||(defined VIEW_HALL_FEEDBACK))
    SPD_CalcAngle(oSpeedSensorAux_M1,MC_NULL, 0, bMotorNbr);  /*  only if auxiliary sensor is enabled */
#endif
  }
  else
  {
...
#if ((defined ENCODER2)||(defined VIEW_ENCODER_FEEDBACK2))
    SPD_CalcAngle(oSpeedSensor[bMotorNbr],MC_NULL, EAC_GetAligned_num(oEAC_M2), bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/
#endif    
#if ((defined HALL_SENSORS2)||(defined VIEW_HALL_FEEDBACK2))
    SPD_CalcAngle(oSpeedSensorAux_M2,MC_NULL, 0, bMotorNbr);  /*  only if auxiliary sensor is enabled */
#endif
  }
  if(STM_GetState(oSTM[bMotorNbr])!=IDLE)
  ...
  else
  {
    if (bMotorNbr == 0)
    {
      ...
      SPD_CalcAngle(oSpeedSensor[bMotorNbr],&STO_Inputs, 0, bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/
...
#endif
#if (defined STATE_OBSERVER_CORDIC)
      SPD_CalcAngle(oSpeedSensor[bMotorNbr],&STO_CR_Inputs, 0, bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/
...
#endif      
#if (defined(AUX_STATE_OBSERVER_PLL))
      SPD_CalcAngle(oSpeedSensorAux_M1,&STO_Inputs, 0, bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/
      
      STO_CalcAvrgElSpeedDpp((CSTO_SPD) oSpeedSensorAux_M1); /*  Only in case of Sensor-less */
#endif
#if (defined(AUX_STATE_OBSERVER_CORDIC))
      SPD_CalcAngle(oSpeedSensorAux_M1,&STO_CR_Inputs, 0, bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/
    ...
    else
    {
      ...
      if((hState == START) || (hState == START_RUN)) /*  only for sensor-less*/
      {
        int16_t hObsAngle;
        hObsAngle = SPD_GetElAngle(oSpeedSensor[bMotorNbr]);
        SPD_CalcAngle(oVSS_M2,&hObsAngle, 0, bMotorNbr);
      }      
#endif
...
#endif      
#if (defined(AUX_STATE_OBSERVER_PLL2))
      SPD_CalcAngle(oSpeedSensorAux_M2,&STO_Inputs, 0, bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/
      
      STO_CalcAvrgElSpeedDpp((CSTO_SPD) oSpeedSensorAux_M2); /*  Only in case of Sensor-less */
#endif
#if (defined(AUX_STATE_OBSERVER_CORDIC2))
      SPD_CalcAngle(oSpeedSensorAux_M2,&STO_CR_Inputs, 0, bMotorNbr);   /* if not sensorless then 2nd parameter is MC_NULL*/
  ...
  if(hMFTaskCounterM1_t > 0u)
  {
    hMFTaskCounterM1_t--;
  }   
  else
  {
	 rt_sem_release(&hMFTask1_sem);
   hMFTaskCounterM1_t = TF_REGULATION_RATE/SPEED_LOOP_FREQUENCY_HZ-1;
  } 
    
#ifdef DUALDRIVE
  if(hMFTaskCounterM2_t > 0u)
  {
    hMFTaskCounterM2_t--;
  }   
  else
  {
		rt_sem_release(&hMFTask2_sem);
    hMFTaskCounterM2_t = TF_REGULATION_RATE2/SPEED_LOOP_FREQUENCY_HZ2-1;
  }
#endif
...
/**
  ...
  */
#pragma inline
extern rt_uint8_t motor_iqcurrent_test;
extern rt_uint8_t motor_idcurrent_test;
...
uint16_t FOC_CurrController(uint8_t bMotor)
{
  ...
  if (STM_GetState(oSTM[bMotor])==ALIGNMENT)
	{
		Vqd.qV_Component1 = FOCVars[bMotor].Iqdref.qI_Component1*3;
//		PI_Controller(oPIDIq[bMotor], 
//							(int32_t)(FOCVars[bMotor].Iqdref.qI_Component1) - 0);
		Vqd.qV_Component2 = FOCVars[bMotor].Iqdref.qI_Component2*3;
//		PI_Controller(oPIDId[bMotor], 
//							(int32_t)(FOCVars[bMotor].Iqdref.qI_Component2) - 0); 
	}
	else
	{
