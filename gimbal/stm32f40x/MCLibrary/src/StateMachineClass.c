...
//  pVars->bState = IDLE; //DRF-UPD
	pVars->bState = IDLE_START;
...
bool STM_NextState(CSTM this, State_t bState)
{
  ...
  case CLEAR:
    if ((bState == START) || (bState == ANY_STOP) || bState == IDLE)
    {
      bNewState = bState;
      bChangeState = TRUE;
    }
                                                                  ...
                                                                  hResetErrors)
{
  ...
  /* Set corrent errors */
  pVars->hFaultNow |= hSetErrors;  
  pVars->hFaultOccurred |= hSetErrors;
  /* Reset current errors */
  pVars->hFaultNow &= (uint16_t)(~hResetErrors);
