...
static int16_t ENC_CalcAngle(CSPD this, void *pInputVars_str, int32_t Aligned_num, uint8_t bMotorNbr);
static bool ENC_CalcAvrgMecSpeed01Hz(CSPD this, int16_t *pMecSpeed01Hz);
static int32_t ENC_SetMecAngle(CSPD this, int16_t hMecAngle);
...
//  TIMx->CNT = 0u;
  ...
  pDVars_str->hSpeedSamplingFreqHz = pDParams_str->hSpeedSamplingFreq01Hz;/// 10u;//DRF-UPD
...
*/
#define SENSOR_POSITION
int32_t sdnaosd1,sdnaosd2;
int16_t sdnaosd3;
int16_t wDelta_angle;
int32_t wOverallAngleVariation = 0;
int32_t wOverallAngleVariation_tmp = 0;
static int16_t ENC_CalcAngle(CSPD this, void *pInputVars_str, int32_t Aligned_num, uint8_t bMotorNbr)
{
  pDParams_t pDParams_str = DCLASS_PARAM;
  pDVars_t pDVars_str = DCLASS_VARS;
  ...
  int16_t htemp1;
  int16_t htemp2;
	rt_bool_t Error_flag = RT_FALSE;
//	float Eroll,Epitch,Eyaw;
	int16_t wNewCounter;
	
  
  uint8_t bBufferIndex = 0u;
  uint8_t bBufferSize = 16;//pDParams_str->bSpeedBufferSize;
	
#ifdef IMU_POSITION 
  sensfusion6GetEulerRPY(&Eroll, &Epitch, &Eyaw);
	
	if (bMotorNbr == 0)
	{
		((_CSPD)this)->Vars_str.wPosition = (int32_t)(Eroll*pDParams_str->hPulseNumber/360) - Aligned_num;
	}
	else
	{
		((_CSPD)this)->Vars_str.wPosition = (int32_t)(Epitch*pDParams_str->hPulseNumber/360) - Aligned_num;
	}
#endif
	
#ifdef SENSOR_POSITION 

		((_CSPD)this)->Vars_str.wPosition = (int32_t)(SensorGetAngle(bMotorNbr,&Error_flag)) - Aligned_num;
	
#endif
	
	if (((_CSPD)this)->Vars_str.wPosition<0)
	{
		((_CSPD)this)->Vars_str.wPosition = ((_CSPD)this)->Vars_str.wPosition + pDParams_str->hPulseNumber;
	}
	
	sdnaosd1 = ((_CSPD)this)->Vars_str.wPosition;
	...
	wtemp1 = (int32_t)(((_CSPD)this)->Vars_str.wPosition) * (int32_t)(pDVars_str->wU32MAXdivPulseNumber);
//  wtemp1 = (int32_t)(pDParams_str->TIMx->CNT) * (int32_t)(pDVars_str->wU32MAXdivPulseNumber);
  sdnaosd2 = wtemp1;
  ...
  ((_CSPD)this)->Vars_str.hMecAngle = htemp2; 
	...
	if(bBufferIndex%bBufferSize == 0)
	{
		if (!((_CSPD)this)->Vars_str.bIs_First_Measurement)
		{
			wNewCounter = ((_CSPD)this)->Vars_str.hMecAngle;//((_CSPD)this)->Vars_str.wPosition;//TIMx->CNT;

			/*Calculation of delta angle*/
			wDelta_angle = (int16_t)(wNewCounter - pDVars_str->hPreviousCapture);
			
//			wOverallAngleVariation_tmp  += wDelta_angle;
//			pDVars_str->wDeltaCapturesBuffer[pDVars_str->bDeltaCapturesIndex]= wDelta_angle;   
		} 
		else
		{
			((_CSPD)this)->Vars_str.bIs_First_Measurement = FALSE;
	//		wNewCounter = TIMx->CNT;
			wNewCounter = ((_CSPD)this)->Vars_str.hMecAngle;//((_CSPD)this)->Vars_str.wPosition;//TIMx->CNT;
		}
		
		pDVars_str->hPreviousCapture = wNewCounter;
  }
...
/**
  ...
  */
extern int16_t eulerspeedActual;
static bool ENC_CalcAvrgMecSpeed01Hz(CSPD this, int16_t *pMecSpeed01Hz) 
{
  pDParams_t pDParams_str = DCLASS_PARAM;
  pDVars_t pDVars_str = DCLASS_VARS;
...
//  TIMx->EGR = pDVars_str->hTIM_EventSource_CCx;
	...
	wtemp1 = (int32_t)(wDelta_angle * pDParams_str->hSpeedSamplingFreq01Hz);
	wtemp1 *= 360;
	wtemp1 = wtemp1>>16;
	
	//  *pMecSpeed01Hz = eulerspeedActual;
//  ((_CSPD)this)->Vars_str.hMecAccel01HzP = (int16_t)(eulerspeedActual - 
//              ((_CSPD)this)->Vars_str.hAvrMecSpeed01Hz);
//	
//	((_CSPD)this)->Vars_str.hAvrMecSpeed01Hz = eulerspeedActual;
...
//  TIMx->CNT = (uint16_t)(hAngleCounts);
	...
	wAngleCounts_bak = ((_CSPD)this)->Vars_str.wPosition - (uint16_t)(hAngleCounts);
	
	if (wAngleCounts_bak<0)
	{
		wAngleCounts_bak = wAngleCounts_bak + pDParams_str->hPulseNumber;
	}	
	sdnaosd2 = wAngleCounts_bak;
	return wAngleCounts_bak;
}
