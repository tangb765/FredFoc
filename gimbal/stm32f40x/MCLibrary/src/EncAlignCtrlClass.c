...
void EAC_Init(CEAC this, CSTC oSTC, CVSS_SPD oVSS, CENC_SPD oENC )
{
  ...
  pVars->oVSS = oVSS;
  pVars->oENC = oENC;
	if(configs.encoder_align_pos == 0)
		pVars->EncAligned = FALSE;
	else
		pVars->EncAligned = TRUE;
  pVars->EncRestart = FALSE;
	pVars->EncAligned_num = configs.encoder_align_pos;
}
...
void EAC_StartAlignment(CEAC this)
{
  ...
  STC_ExecRamp(pVars->oSTC, pParams->hFinalTorque, pParams->hDurationms);
...
bool EAC_Exec(CEAC this)
{
  ...
  if (pVars->hRemainingTicks > 0u)
  {
    ...
    {
      /* Set oVSS mechanical angle.*/
			configs.encoder_align_pos = SPD_SetMecAngle((CSPD)pVars->oENC, pParams->hElAngle / (int16_t)(pParams->bElToMecRatio));
      pVars->EncAligned_num  = configs.encoder_align_pos;//SPD_SetMecAngle((CSPD)pVars->oENC, pParams->hElAngle / (int16_t)(pParams->bElToMecRatio));
      pVars->EncAligned = TRUE;
      retVal = TRUE;
    }
    else
...
int32_t EAC_GetAligned_num(CEAC this)
{
	pVars_t pVars = CLASS_VARS;
	return pVars->EncAligned_num;
}

void EAC_SetAligned_num(CEAC this, int32_t Aligned_num)
{
  pVars_t pVars = CLASS_VARS;
  pVars->EncAligned_num = Aligned_num;
}
