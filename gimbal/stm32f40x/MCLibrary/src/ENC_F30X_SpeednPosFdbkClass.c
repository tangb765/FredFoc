...
*/
#define AS504X_POSITION
int32_t sdnaosd1,sdnaosd2,sdnaosd3,sdnaosd4,sdnaosd5;
static int16_t ENC_CalcAngle(CSPD this, void *pInputVars_str, int32_t Aligned_num, uint8_t bMotorNbr)
{
  pDParams_t pDParams_str = DCLASS_PARAM;
  pDVars_t pDVars_str = DCLASS_VARS;
...
#ifdef AS504X_POSITION 

		((_CSPD)this)->Vars_str.wPosition = (int32_t)(AS504XGetAngle(bMotorNbr,&Error_flag)) - Aligned_num;
...
//  TIMx->EGR = pDVars_str->hTIM_EventSource_CCx;
 ...
 if (!((_CSPD)this)->Vars_str.bIs_First_Measurement)
  {
		wNewCounter = ((_CSPD)this)->Vars_str.wPosition;//TIMx->CNT;

    /*Calculation of delta angle*/
		wDelta_angle = (int32_t)(wNewCounter) - (int32_t)(pDVars_str->hPreviousCapture);
		

		if (wDelta_angle>(pDParams_str->hPulseNumber/2))
		{
			wDelta_angle = wDelta_angle - pDParams_str->hPulseNumber;
		}
		else if (wDelta_angle<-(pDParams_str->hPulseNumber/2))
		{
			wDelta_angle = wDelta_angle + pDParams_str->hPulseNumber;
		}
...
//		wNewCounter = TIMx->CNT;
  ...
  wtemp1 = wOverallAngleVariation * (int32_t)(pDParams_str->hSpeedSamplingFreq01Hz);//*6; // 10*MEDIUM_FREQUENCY_TASK_RATE 10*500
//  wtemp1 = wOverallAngleVariation * (int32_t)(pDParams_str->hSpeedSamplingFreq01Hz)*6;
  wtemp2 = (int32_t)(pDParams_str->hPulseNumber)*
    (int32_t)(pDParams_str->bSpeedBufferSize);//4000*16
  wtemp1 /= wtemp2;  
	wtemp1 *= 10; 
