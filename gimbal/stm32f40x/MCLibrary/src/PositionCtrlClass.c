...
/* Includes ------------------------------------------------------------------*/
#include "PositionCtrlClass.h"
#include "PositionCtrlPrivate.h"
...
void STC_SetMecSpeedRef01Hz(CSTC this,int16_t MecSpeed)
{
	((_CSTC)this)->Vars_str.wSpeedRef01HzExt = (int32_t)(MecSpeed*65536);
}
...
void STC_SetControlMode(CSTC this, STC_Modality_t bMode)
{
  pVars_t pVars = CLASS_VARS;
  pVars->bMode = bMode;
  pVars->hRampRemainingStep = 0u; /* Interrupts previous ramp. */
}
...
bool STC_ExecRamp(CSTC this, int16_t hTargetFinal, uint16_t hDurationms)
{
  ...
  if (AllowedRange == TRUE)
  {
    ...
    else
    {
      ...
      /* Compute the (hRampRemainingStep) number of steps remaining to complete 
      the ramp. */
      wAux = (uint32_t)hDurationms * (uint32_t)pParams->hSTCFrequencyHz;
      wAux /= 1000u;
      pVars->hRampRemainingStep = (uint16_t)(wAux);
      pVars->hRampRemainingStep++;
      ...
      wAux1 /= (int32_t)pVars->hRampRemainingStep;
...
int16_t STC_CalcTorqueReference(CSTC this)
{
  ...
  int16_t hError;
  int16_t hMeasuredPosition;
  int16_t hTargetPosition;
	int16_t hErrorPosition;
  ...
  if (pVars->hRampRemainingStep > 1u)
  {
  ...
  else if (pVars->hRampRemainingStep == 1u)
  {
  ...
  if (pVars->bMode == STC_SPEED_MODE)
  {
		...
		hMeasuredPosition = 0;
		hTargetPosition = 0;
		hErrorPosition = hTargetPosition - hMeasuredPosition;	
		
		hTargetSpeed = PI_Controller(pVars->oPISpeed, (int32_t)hError);
...
bool STC_RampCompleted(CSTC this)
{
  pVars_t pVars = CLASS_VARS;
  bool retVal = FALSE;
  if (pVars->hRampRemainingStep == 0u)
  {
    retVal = TRUE;
  }
  return retVal;
