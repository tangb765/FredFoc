...
#define BUFSIZE  64 //RT_CONSOLEBUF_SIZE
...
static void uart1_tx_idle_hook()
{
	...
	if (uart1_tx_fifo.get_index != uart1_tx_fifo.put_index && DMA_GetCmdStatus(uart1.tx_dma_channel) == DISABLE)
	{
		rt_size_t count=0;
		rt_size_t length=0; 					
		rt_size_t ch=0; 
		...
		/* set buffer address */
		uart1.tx_dma_channel->M0AR = (rt_uint32_t) txbuf;
		
		/* set size */
		uart1.tx_dma_channel->NDTR = length;
		/* enable DMA */
...
static void uart3_tx_idle_hook()
{
	...
	if (uart3_tx_fifo.get_index != uart3_tx_fifo.put_index && DMA_GetCmdStatus(uart3.tx_dma_channel) == DISABLE)
	{
		rt_size_t count=0;
		rt_size_t length=0; 					
		rt_size_t ch=0; 
		...
		/* set buffer address */
		uart3.tx_dma_channel->M0AR = (rt_uint32_t) tx3buf;
		/* set size */
		uart3.tx_dma_channel->NDTR = length;
		/* enable DMA */
...
void USART1_IRQHandler(void)
{
    ...
    {  
			  ...
			  rt_size_t len_i = 0; 
				...
				DMA_ClearFlag(UART1_RX_DMA,DMA_FLAG_TCIF5);  
...
void DMA2_Stream7_IRQHandler(void)
{
    /* enter interrupt */
    rt_interrupt_enter();
    if (DMA_GetITStatus(UART1_TX_DMA, DMA_IT_TCIF7) != RESET)
    {
				DMA_Cmd(UART1_TX_DMA, DISABLE);
        DMA_ClearFlag(UART1_TX_DMA, DMA_FLAG_TCIF7);
    }
...
void USART3_IRQHandler(void)
{
    ...
    {  
			  ...
			  rt_size_t len_i = 0; 
				...
				DMA_ClearFlag(UART3_RX_DMA,DMA_FLAG_TCIF1);  
...
void DMA1_Stream3_IRQHandler(void)
{
    /* enter interrupt */
    rt_interrupt_enter();
    if (DMA_GetITStatus(UART3_TX_DMA, DMA_IT_TCIF3) != RESET)
    {
				DMA_Cmd(UART3_TX_DMA, DISABLE);
        DMA_ClearFlag(UART3_TX_DMA, DMA_FLAG_TCIF3);
    }
    /* leave interrupt */
    rt_interrupt_leave();
}
...
static void DMA_Configuration(void)
{
    ...
    DMA_InitStructure.DMA_Channel = DMA_Channel_4;  
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&USART3->DR);  
    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)rx3buf;  
    DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;  
    DMA_InitStructure.DMA_BufferSize = BUFSIZE;  
    DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;  
    DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;  
